...
## What we already know (verified on the Windows PC, 2026-09-08)

From `inspect --windows` on the real machine:

```
title="Victory Bazars - [Report Viewer]"
class="WindowsForms10.Window.8.app.0.fb11c8_r6_ad1"
pid=14344
```

* NS Retail is a **WinForms MDI application**: screens open as child windows
  *inside* the main window, and the main window's title changes to
  `Victory Bazars - [<active screen>]`.
* The Report Viewer is therefore **not** a separate top-level window. Match it
  by title instead:
  * `windows.main` -> `"title_re": "^Victory Bazars"`
  * `windows.report_viewer` -> `"title_re": "Victory Bazars - \\[Report Viewer\\]"`
* The class name suffix (`fb11c8_r6_ad1`) is generated by .NET and can change
  between versions - **match on the title, not the class name**.
* The save dialog is expected to be a real top-level window (standard Windows
  dialog, class `#32770`) - still to be confirmed.
...
```bat
REM capture the NS Retail window directly (no clicking, most reliable):
scripts\inspect_windows.bat reports_menu "Victory Bazars.*"

REM or capture whatever you click during an 8 second countdown:
scripts\inspect_windows.bat reports_menu
```

Each capture is saved to `docs\captures\<name>.txt` (readable tree) and
`docs\captures\<name>.json` (full detail).
